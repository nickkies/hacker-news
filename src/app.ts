...
abstract class View {
  ...
  protected constructor(containerId: string, template: string) {
...
class NewsFeedView extends View {
  ...
  render(): void {
    ...
    const cp = store.currentPage,
      tp = Math.ceil(this.feeds.length/10);
  
    this.setTemplateData('news_feed', this.getHtml());
    this.setTemplateData('prev_page', String(cp > 1 ? cp - 1 : 1));
    this.setTemplateData('next_page', String(cp < tp ? cp + 1 : tp));
